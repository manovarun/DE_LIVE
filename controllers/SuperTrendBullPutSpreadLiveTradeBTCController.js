...
const FUT_CANDLES_COLLECTION =
  process.env.SUPERTREND_BPS_LIVE_FUT_CANDLES_COLLECTION ||
  'btcusd_candles_ts';
...
const SQUARE_OFF_TIME = process.env.SUPERTREND_BPS_LIVE_SQUARE_OFF_TIME || '23:59';
...
const STOPLOSS_ENABLED =
  String(process.env.SUPERTREND_BPS_LIVE_STOPLOSS_ENABLED || 'true').toLowerCase() ===
  'true';
const STOPLOSS_PCT = Number(process.env.SUPERTREND_BPS_LIVE_STOPLOSS_PCT || 5000);
...
const HEDGE_MONEYNESS = process.env.SUPERTREND_BPS_LIVE_HEDGE_MONEYNESS || 'OTM3';

const ATR_PERIOD = Math.max(1, Number(process.env.SUPERTREND_BPS_LIVE_ATR_PERIOD || 10));
const MULTIPLIER = Math.max(0.1, Number(process.env.SUPERTREND_BPS_LIVE_MULTIPLIER || 3));
const CHANGE_ATR_CALC =
  String(process.env.SUPERTREND_BPS_LIVE_CHANGE_ATR_CALC || 'true').toLowerCase() ===
  'true';

const FORCE_ENTRY =
  String(process.env.SUPERTREND_BPS_LIVE_FORCE_ENTRY || 'false').toLowerCase() ===
  'true';

const WEEKDAYS = (process.env.SUPERTREND_BPS_LIVE_WEEKDAYS || 'MON,TUE,WED,THU,FRI,SAT,SUN')
  .split(',')
...
const MIN_CANDLES = Math.max(5, Number(process.env.SUPERTREND_BPS_LIVE_MIN_CANDLES || 20));
...
const ALLOW_FORMING_CANDLE =
  String(process.env.SUPERTREND_BPS_LIVE_ALLOW_FORMING_CANDLE || 'false').toLowerCase() ===
  'true';
...
async function deltaRequest(method, path, { query, body, auth } = {}) {
  ...
  const headers = { 'User-Agent': DELTA_USER_AGENT, Accept: 'application/json' };
...
function enrichSupertrendCandles(stCandles, stockSymbol, stockName, timeInterval) {
  return (stCandles || []).map((c) => ({ ...c, stockSymbol, stockName, timeInterval }));
}
...
async function pickPeLegsFromTicks({ optColl, expiry, underlyingPrice, mainMoneyness, hedgeMoneyness, asOfUtc }) {
  const strikes = await getAvailableStrikesForExpiry(optColl, { underlying: 'BTC', optionType: 'P', expiry });
  if (!strikes.length) return { ok: false, reason: 'NO_STRIKES_FOR_EXPIRY', strikes: [] };

  const mainStrike = pickStrikeByMoneyness(strikes, underlyingPrice, mainMoneyness, 'P');
  const hedgeStrike = pickStrikeByMoneyness(strikes, underlyingPrice, hedgeMoneyness, 'P');
  ...
  const mainTick = await getLatestOptionTickByStrike(optColl, { underlying: 'BTC', optionType: 'P', expiry, strike: mainStrike, atOrBeforeUtc: asOfUtc });
  const hedgeTick = await getLatestOptionTickByStrike(optColl, { underlying: 'BTC', optionType: 'P', expiry, strike: hedgeStrike, atOrBeforeUtc: asOfUtc });
  ...
  return {
    ok: true,
    strikes,
    main: { symbol: mainTick.symbol, strike: Number(mainTick.strike), optionType: 'P', ltp: Number(mainTick.price), tickTime: mainTick.exchTradeTime },
    hedge: { symbol: hedgeTick.symbol, strike: Number(hedgeTick.strike), optionType: 'P', ltp: Number(hedgeTick.price), tickTime: hedgeTick.exchTradeTime },
    mainStrike,
    hedgeStrike,
  };
}
...
async function executeExit({ runId, open, reason, nowIst }) {
  ...
  return { reason, time: nowIst.toISOString(true), orders: { mainExitOrder, hedgeExitOrder }, prices: { mainExitPrice, hedgeExitPrice }, pnl, tookMs };
}
...
async function maybeEnterTrade(nowIst) {
  ...
  const buySignal = !!(signalCandle?.supertrend?.buySignal ?? signalCandle?.buySignal);
  if (!FORCE_ENTRY && !buySignal) return { took: false, reason: 'NO_BUY_SIGNAL' };
  ...
  const entryDoc = await createSpreadDoc({
    ...
    main: { side: 'SELL', optionType: 'P', symbol: legs.main.symbol, strike: legs.main.strike, entry: { time: nowIst.toISOString(true), price: Number(legs.main.ltp) } },
    hedge: { side: 'BUY', optionType: 'P', symbol: legs.hedge.symbol, strike: legs.hedge.strike, entry: { time: nowIst.toISOString(true), price: Number(legs.hedge.ltp) } },
    exit: null,
    pnl: null,
  });
  ...
  info(`ENTRY expiry=${expiry} buySignal=${buySignal} underlying=${underlyingPrice.toFixed(1)} main=${legs.main.symbol}@${exec.main.fillPrice} hedge=${legs.hedge.symbol}@${exec.hedge.fillPrice} qty=${QTY}`);
...
const SuperTrendBullPutSpreadLiveTradeBTCController = expressAsyncHandler(async (req, res, next) => {
  try {
    const nowIst = moment().tz(TZ);
    const expiry = getActiveDailyExpiry(nowIst);
    const open = await findOpenSpreadForExpiry(expiry);
