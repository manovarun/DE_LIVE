...
const LOTS = Math.max(
  1,
  parseInt(process.env.SUPERTREND_BPS_LIVE_LOTS || '1', 10)
);
...
async function executeEntry({ runId, expiry, underlyingPrice, legs, nowIst }) {
  ...
  const hedgeClientId = makeClientOrderId('STBPS', runId, 'HB');
  const mainClientId = makeClientOrderId('STBPS', runId, 'MS');
...
async function executeExit({ runId, open, reason, nowIst }) {
  ...
  const mainClientId = makeClientOrderId('STBPS', runId, 'MB');
  const hedgeClientId = makeClientOrderId('STBPS', runId, 'HS');
