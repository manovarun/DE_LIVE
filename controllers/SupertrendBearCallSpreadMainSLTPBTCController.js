...
// ATM selection for CALL ladder (prefer strike >= underlying on tie)
function pickAtmCallIndex(calls, underlying) {
  if (!Array.isArray(calls) || !calls.length) return -1;
  if (!Number.isFinite(underlying)) return -1;

  let bestIdx = 0;
  let bestDiff = Math.abs(calls[0].strike - underlying);

  for (let i = 1; i < calls.length; i += 1) {
    const d = Math.abs(calls[i].strike - underlying);
    if (d < bestDiff) {
      bestDiff = d;
      bestIdx = i;
    } else if (d === bestDiff) {
      // tie-break: prefer >= underlying for ATM-calls ladder
      const aGe = calls[bestIdx].strike >= underlying ? 1 : 0;
      const bGe = calls[i].strike >= underlying ? 1 : 0;
      if (bGe > aGe) bestIdx = i;
    }
  }
...
// CE ladder rules (ascending strikes):
//   ITM calls: lower strikes => atmIndex - steps
//   OTM calls: higher strikes => atmIndex + steps
function pickCallByMoneyness(calls, atmIndex, spec, legLabel) {
  const parsed = parseMoneynessSpec(spec);
  const { type, steps, raw } = parsed;
  ...
  return { inst: calls[idx], parsed, reason: null };
}
...
// -----------------------------------------------------------------------------
// Single-run executor: BTC Supertrend Bear Call Spread
// -----------------------------------------------------------------------------
async function runSupertrendBearCallSingleRunBTC(params) {
  const {
    fromDate,
    toDate,
  ...
  const getCallsForExpiry = async (expiryYMD) => {
    if (!expiryYMD) return [];
    if (optionChainCache.has(expiryYMD)) return optionChainCache.get(expiryYMD);

    const { calls } = await fetchOptionChainFromTicks({
      optColl,
      asset: stockName,
      currency: 'USD',
    ...
    optionChainCache.set(expiryYMD, calls || []);
    return calls || [];
  };
  ...
  if (expiryModeNorm === 'FIXED') {
    callsFixed = await getCallsForExpiry(expiryFixed);
    if (!callsFixed.length && VERBOSE) {
      console.log(
        `[${TAG}] No CALL instruments found in delta_options_ts for expiry=${expiryFixed}.`
      );
    }
  }
  ...
  for (
    ...
    if (expiryModeNorm === 'FIXED' && (!callsFixed || !callsFixed.length)) {
      results.push({
        date: currentDate,
    ...
    while (scanStartIdx < dayCandles.length) {
      ...
      // Entry: first sellSignal after scanStartIdx
      let entryIdx = dayCandles.findIndex(
        (c, idx) =>
          idx >= scanStartIdx && c?.supertrend && !!c.supertrend.sellSignal
      );
      let entryReasonTag = 'SUPERTREND_SELL_SIGNAL';

      // Fallback: if no sellSignal but trend already DOWN (only for first trade of the day)
      if (
        entryIdx === -1 &&
        ...
        if (fallbackIdx !== -1) {
          ...
          entryReasonTag = 'SUPERTREND_DOWNTREND_NO_SELL_SIGNAL';
        }
      }
      ...
      // Exit: first buySignal after entry; else timeExitHHmm candle; else last candle
      let exitIdx = dayCandles.findIndex(
        (c, idx) => idx > entryIdx && c?.supertrend && !!c.supertrend.buySignal
      );
      ...
      const callsRef =
        expiryModeNorm === 'FIXED'
          ? callsFixed || []
          : await getCallsForExpiry(expiryUsed);

      if (!callsRef || !callsRef.length) {
        results.push({
          date: currentDate,
          trade: { took: false, reason: 'NO_CALL_CHAIN' },
      ...
      // Select strikes
      const atmIdx = pickAtmCallIndex(callsRef, underlyingPrice);
      if (atmIdx < 0) {
        results.push({
          date: currentDate,
          trade: { took: false, reason: 'NO_ATM_CALL' },
      ...
      const mainPick = pickCallByMoneyness(
        callsRef,
        atmIdx,
        mainMoneyness,
        'MAIN_CALL'
      );
      const hedgePick = pickCallByMoneyness(
        callsRef,
        atmIdx,
        hedgeMoneyness,
        'HEDGE_CALL'
      ...
      if (!mainPick.inst) {
        results.push({
          ...
          trade: {
            took: false,
            reason: mainPick.reason || 'MAIN_CALL_NOT_FOUND',
          },
      ...
      if (!hedgePick.inst) {
        results.push({
          ...
          trade: {
            took: false,
            reason: hedgePick.reason || 'HEDGE_CALL_NOT_FOUND',
          },
      ...
      if (!mainEntry || typeof mainEntry.price === 'undefined') {
        results.push({
          ...
          ceLeg: { main: mainPick.inst, hedge: hedgePick.inst },
        });
        scanStartIdx = exitIdx + 1;
        continue;
      }
...
// -----------------------------------------------------------------------------
// Controller: BTC Supertrend Bear Call Spread backtest (Delta ticks + BTC candles)
// -----------------------------------------------------------------------------
exports.SupertrendBearCallSpreadMainSLTPBTCController = expressAsyncHandler(
  async (req, res) => {
    try {
      const payload = req.body || {};
      if (VERBOSE) console.log('BTC_ST_BCS payload=', payload);
      ...
      return res.status(200).json({
        success: true,
        strategy: 'SUPERTREND_BEAR_CALL_SPREAD_MAIN_SLTP_BTC_DELTA',
        collections: {
      ...
      console.error(
        '[SupertrendBearCallSpreadMainSLTPBTCController] Error:',
        err
      );
      return res.status(400).json({
        success: false,
        message:
          err.message || 'Error running BTC Supertrend BCS backtest controller',
      });
    }
  }
);
