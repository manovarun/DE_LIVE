      ...
      for (const exitTime of exitTimes) {
        ...
        for (
          ...
          try {
            ...
            // 🟢 Define the Strike Prices for Short Strangle
            const otmCEStrikePrice = baseStrikePrice + otmOffset;
            const otmPEStrikePrice = baseStrikePrice - otmOffset;
            ...
            if (!callOptionSell || !putOptionSell) {
              console.warn(`Option data missing for ${date}. Skipping.`);
              continue;
            }
            ...
            console.log(
              `otmCEStrikePrice:${otmCEStrikePrice},  ceEntryPrice: ${ceEntryPrice}`
            );
            console.log(
              `otmPEStrikePrice:${otmPEStrikePrice},  peEntryPrice: ${peEntryPrice}`
            );

            // Fetch Exit Prices for short legs
            const ceExitData = await HistoricalOptionData.findOne({
              timeInterval,
              datetime: exitTimeStr,
              expiry,
            ...
            console.log(
              `otmCEStrikePrice:${otmCEStrikePrice},  ceExitPrice: ${ceExitPrice}`
            );
            console.log(
              `otmPEStrikePrice:${otmPEStrikePrice},  peExitPrice: ${peExitPrice}`
            );

            // Fetch VIX Data (Optional)
            const vixData = await HistoricalIndicesData.findOne({
              timeInterval,
              datetime: entryTimeStr,
              stockSymbol: 'India VIX',
        ...
        allResults.push({ strategyId, results: results.reverse() });
      }
    }
