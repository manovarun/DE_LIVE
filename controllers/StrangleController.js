...
exports.OTMShortStrangleVix = expressAsyncHandler(async (req, res, next) => {
  try {
    const {
      timeInterval,
      fromDate,
        ...
        for (const exitTime of exitTimes) {
          ...
          for (
            ...
            try {
              ...
              const ceExitPrice =
                ceExitData.length > 0 ? ceExitData[0].close : ceEntryPrice;
              const peExitPrice =
                peExitData.length > 0 ? peExitData[0].close : peEntryPrice;
