...
//OPTIONS BREAKOUTS FIRST MINUTE backtestBreakoutCandleNios
exports.backtestBreakoutIndicesTick = expressAsyncHandler(
  async (req, res, next) => {
    const {
      fromDate,
      toDate,
...
//FUTURES BREAKOUTS FIRST MINUTE
exports.backtestBreakoutFuturesTick = expressAsyncHandler(
  async (req, res, next) => {
    const {
      fromDate,
      toDate,
    ...
    for (
      ...
      const firstCandleAgg = await MarketData.aggregate([
        {
          $match: {
            ...
            exchange: 'NFO',
            exchTradeTime: {
              $gt: candleStart.format('YYYY-MM-DDTHH:mm:ssZ'),
              $lt: candleEnd.format('YYYY-MM-DDTHH:mm:ssZ'),
            },
          },
        },
      ...
      console.log('Breakout Tick LTP', breakoutTick.ltp);
...
exports.backtestBreakoutFuturesCandle = expressAsyncHandler(
  async (req, res, next) => {
    const {
      fromDate,
      toDate,
...
//FUTURES BASED OPTION BREAKOUTS FIRST MINUTE
exports.backtestBreakoutFuturesOptionsTick = expressAsyncHandler(
  async (req, res, next) => {
    const {
      fromDate,
      toDate,
    ...
    for (
      ...
      const nearestStrike =
        Math.round(breakoutTick.ltp / strikeInterval) * strikeInterval;
      console.log('Nearest Strike', nearestStrike);
      ...
      const entryTick = await MarketData.findOne({
        symbolToken: optionToken.token,
        exchTradeTime: { $lte: breakoutTick.exchTradeTime },
      })
        .sort({ exchTradeTime: -1 })
        .lean();
      if (!entryTick) continue;
      console.log('entryTick', entryTick);
        ...
        $or: [
          {
            exchTradeTime: {
              $exists: true,
              $ne: null,
              $gte: breakoutTick.exchTradeTime,
              $lte: backtestEnd.format('YYYY-MM-DDTHH:mm:ssZ'),
            },
          },
          {
            exchTradeTime: { $exists: false },
            exchFeedTime: {
              $exists: true,
              $ne: null,
              $gte: breakoutTick.exchFeedTime,
